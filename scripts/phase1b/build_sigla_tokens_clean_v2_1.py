...
def canonical_sigla_latin_side_base(s: str) -> str | None:
    """Return an unsuffixed lookup key for a lowercase Latin side marker.

    SigLA sometimes distinguishes faces as e.g. ``KN Wb 33a`` / ``33b`` while
    the diplomatic source has a single combined document ``KNWb33``. This
    alias is admissible only when the *exact unsuffixed diplomatic key exists*.
    We do not strip Latin suffixes from diplomatic keys, so ``HT 41b`` cannot
    silently alias a distinct diplomatic ``HT41a``.
    """
    raw = unicodedata.normalize("NFKC", (s or "").strip())
    compact = re.sub(r"[^0-9A-Za-zΑ-Ωα-ω]+", "", raw)
    if re.search(r"\d[a-z]$", compact):
        return compact[:-1].upper()
    return None
...
def resolve_doc(sigla_id: str, exact, base) -> tuple[str | None, str]:
    ...
    if len(cands) > 1:
        return None, "AMBIGUOUS_DOC"
    # Candidate-blind face alias: only a lowercase Latin side suffix on the
    # SigLA key, and only to an exact unsuffixed diplomatic document.
    kl = canonical_sigla_latin_side_base(sigla_id)
    if kl is not None:
        cands = exact.get(kl, [])
        if len(cands) == 1:
            return cands[0], "LATIN_SIDE_TO_UNSUFFIXED_DOC"
        if len(cands) > 1:
            return None, "AMBIGUOUS_DOC"
