# Multi-ending stem grid v0.1

Source: frozen v0.5 accepted suffix pairs, with **2** post-v0.5 forward source controls masked.

Active source-consistent suffix cells after forward masking: **43**. Multi-ending stems (2+ endings): **7**. Two-stem overlaps sharing 2+ endings: **0**.

## Multi-ending stems

| stem | endings | count | A/B audited cells | reviewed | cells |
|---|---|---:|---:|---:|---|
| I-DA | A|DA|MI | 3 | 1 | 1 | A:I-DA-A / DA:I-DA-DA / MI:I-DA-MI[A] |
| KU-PA | JA|RI|ZU | 3 | 1 | 1 | JA:KU-PA-JA[A] / RI:KU-PA-RI / ZU:KU-PA-ZU |
| A-DA | RA|RO | 2 | 1 | 2 | RA:A-DA-RA[C] / RO:A-DA-RO[B] |
| MI-DA | NI|RA | 2 | 1 | 1 | NI:MI-DA-NI / RA:MI-DA-RA[B] |
| PA3-NI | NA|WI | 2 | 1 | 1 | NA:PA3-NI-NA / WI:PA3-NI-WI[B] |
| A-RI | PA|SU | 2 | 0 | 2 | PA:A-RI-PA[C] / SU:A-RI-SU[C] |
| KU-PA3 | NU|PA3 | 2 | 0 | 0 | NU:KU-PA3-NU / PA3:KU-PA3-PA3 |

## Shared-ending overlaps

No pair of multi-ending stems shares two or more endings in the current structurally retained matrix.

## Promotion rule

A formal paradigm grid requires a second independent stem sharing at least two endings, with source-secure cells and the manual Tier A/B threshold registered in `experiments/multi-ending-grid-v01.md`. A single multi-ending stem is not promoted as a grammatical paradigm.
